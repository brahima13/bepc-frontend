...
export default function App() {
  ...
  const handleImageSelected = async (file) => {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        const res = await fetch('https://bepc-backend.onrender.com/api/scan', {
          method: 'POST',
          headers: {
        ...
        if (data.result) {
          try {
            ...
            setScanError('Extraction impossible. Vérifiez la qualité de la photo.')
          }
        }
      } catch {
  ...
  const applyScanData = () => {
    if (!scanData) return
    const dayIdx = scanData.day != null ? Math.min(Math.max(scanData.day - 1, 0), 3) : activeDay
    const session = scanData.session || activeSession
    const newNotes = {}
    NOTES.forEach(n => { newNotes[n] = String(scanData.notes?.[n] ?? 0) })
    const days = form.days.map((d, i) =>
      i === dayIdx ? { ...d, [session]: newNotes } : d
    )
    let newForm = { ...form, days }
    if (scanData.discipline) newForm.discipline = scanData.discipline
    if (scanData.correcteur) newForm.correcteur = scanData.correcteur
    setForm(newForm)
    setActiveDay(dayIdx)
  ...
  const C = {
    ...
    green: '#4ade80', yellow: '#f59e0b', red: '#f87171', purple: '#8b5cf6'
  }
    ...
    <div style={{ fontFamily: 'sans-serif', minHeight: '100vh', background: C.bg, color: C.text }}>
      <header style={{ background: '#0d2137', borderBottom: `2px solid ${C.accent}`, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => document.getElementById('scanGalerie').click()}
            style={{ padding: '8px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12, background: C.purple, color: '#fff' }}>
            🖼️ Galerie
          </button>
      ...
      {scanModal && (
        <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: C.card, borderRadius: 16, padding: 24, maxWidth: 500, width: '100%', border: `1px solid ${C.border}`, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ fontWeight: 900, fontSize: 16, color: C.text, marginBottom: 16 }}>📸 Extraction des données</div>
            {scanImage && (
              <img src={scanImage} alt="Apercu" style={{ width: '100%', maxHeight: 200, objectFit: 'contain', borderRadius: 8, marginBottom: 16, border: `1px solid ${C.border}` }} />
            )}
            {scanLoading && (
              <div style={{ textAlign: 'center', padding: 20, color: C.purple }}>
                <div style={{ fontSize: 32 }}>⚙️</div>
                <div style={{ marginTop: 8, fontWeight: 700 }}>Analyse en cours par IA...</div>
              </div>
            )}
            {scanError && (
              <div style={{ background: '#450a0a', borderRadius: 8, padding: 12, color: C.red, fontSize: 13, marginBottom: 12 }}>
                ⚠️ {scanError}
              </div>
            )}
            {scanData && !scanLoading && (
              <div>
                <div style={{ fontWeight: 800, color: C.green, marginBottom: 12 }}>✅ Données extraites — Modifiez si nécessaire :</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                  <div>
                    <label style={{ fontSize: 10, color: C.muted, fontWeight: 700, display: 'block', marginBottom: 4 }}>DISCIPLINE</label>
                    <input value={scanData.discipline || ''} onChange={e => setScanData(d => ({ ...d, discipline: e.target.value }))} style={inp} />
                  </div>
                  <div>
                    <label style={{ fontSize: 10, color: C.muted, fontWeight: 700, display: 'block', marginBottom: 4 }}>JOUR (1-4)</label>
                    <input type="number" min="1" max="4" value={scanData.day || ''} onChange={e => setScanData(d => ({ ...d, day: parseInt(e.target.value) || 1 }))} style={inp} />
                  </div>
                  <div>
                    <label style={{ fontSize: 10, color: C.muted, fontWeight: 700, display: 'block', marginBottom: 4 }}>SESSION</label>
                    <select value={scanData.session || 'matin'} onChange={e => setScanData(d => ({ ...d, session: e.target.value }))} style={{ ...inp }}>
                      <option value="matin">Matin</option>
                      <option value="soir">Soir</option>
                    </select>
                  </div>
                </div>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, marginBottom: 8 }}>NOTES EXTRAITES :</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 16 }}>
                  {NOTES.map(n => (
                    <div key={n} style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 9, color: n < 10 ? C.red : n < 12 ? C.yellow : C.green, fontWeight: 700 }}>{n}/20</div>
                      <input type="number" min="0"
                        value={scanData.notes?.[n] ?? 0}
                        onChange={e => setScanData(d => ({ ...d, notes: { ...d.notes, [n]: parseInt(e.target.value) || 0 } }))}
                        style={{ width: '100%', padding: '3px 2px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4, color: C.accent, fontWeight: 800, textAlign: 'center', fontSize: 12, boxSizing: 'border-box', outline: 'none' }} />
                    </div>
                  ))}
                </div>
                <button onClick={applyScanData}
                  style={{ width: '100%', padding: '12px 0', borderRadius: 10, border: 'none', background: C.green, color: '#0f172a', fontWeight: 900, fontSize: 15, cursor: 'pointer', marginBottom: 10 }}>
                  ✅ Appliquer ces données
                </button>
              </div>
            )}
            <button onClick={() => { setScanModal(false); setScanImage(null); setScanData(null); setScanError('') }}
              style={{ width: '100%', padding: '10px 0', borderRadius: 10, border: `1px solid ${C.border}`, background: 'transparent', color: C.muted, fontWeight: 700, cursor: 'pointer' }}>
              Fermer
            </button>
          </div>
        </div>
      ...
      {view === 'saisie' && (
        <div style={{ maxWidth: 940, margin: '0 auto', padding: '24px 14px' }}>
          ...
          <div style={{ background: C.card, borderRadius: 14, padding: 22, marginBottom: 18, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 12, color: C.purple, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 16 }}>📅 Saisie par Jour et Session</div>
